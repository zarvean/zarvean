...
const AdminShopManager = () => {
  console.log('AdminShopManager component rendered');
  ...
  const handleUpdateCategory = () => {
    if (editCategoryValue.trim() && editingCategory) {
      console.log('Updating category:', editingCategory, 'to:', editCategoryValue.trim());
      updateCategory(editingCategory, editCategoryValue.trim());
      setEditingCategory('');
      setEditCategoryValue('');
      toast({
        title: "Success",
        description: `Category updated to "${editCategoryValue.trim()}" successfully`
      });
    }
  };
  ...
  const openSectionDialog = (section) => {
    if (section) {
  ...
  const handleSectionSubmit = () => {
    ...
    if (editingSection) {
      console.log('Updating section:', editingSection.id, 'with data:', sectionForm);
      updateSection(editingSection.id, sectionForm);
      toast({
        title: "Success",
        description: `Section "${sectionForm.name}" updated successfully`
      });
    } else {
      console.log('Creating new section:', sectionForm);
      addSection(sectionForm);
      toast({
        title: "Success",
        description: `Section "${sectionForm.name}" created successfully`
      });
    }
  ...
  const handleDeleteSection = (id, sectionName) => {
    showDeleteConfirm({
      title: "Delete Section",
      description: "Are you sure you want to delete this section? This action cannot be undone.",
      itemName: sectionName,
      onConfirm: () => {
        console.log('Deleting section:', id, sectionName);
        removeSection(id);
        toast({
          title: "Success",
  ...
  const toggleSectionCategory = (category) => {
    setSectionForm(prev => ({
      ...prev,
